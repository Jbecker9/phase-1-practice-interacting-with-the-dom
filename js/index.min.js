...
const newInterval = setInterval(setTime, 1000);
...
heart.addEventListener("click", () => {
    ...
    let numClicks = 0
    let li = document.getElementById(`list-${num}`)
    numClicks++
    if (li){
        let obj = {}
        // li.innerText = `${num} has been liked ${numClicks} time's`;
        let sentence = li.innerText.split(' ')
        numClicks = sentence[4]++
...
pause.addEventListener("click", () => {
    let num = counter.innerText
    let timerData = counter.dataset
    let timerString = JSON.stringify([timerData])
    if (timerString.length++){
        clearInterval(newInterval) = true
    }
    else if (timerString.length = num) {
        clearInterval(newInterval) = false
    }
